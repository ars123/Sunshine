package com.example.user.sunshine.sync;
...
import com.example.user.sunshine.BuildConfig;
import com.example.user.sunshine.ForecastAdapter;
import com.example.user.sunshine.MainActivity;
import com.example.user.sunshine.R;
import com.example.user.sunshine.Utility;
...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
  ...
  //  DAY_IN_MILLIS is the amount of milliseconds in a day and WEATHER_NOTIFICATION_ID is an id
  // you create that is matched to your notification so that you can reuse it.
  // If you reuse the notification ID, your application will post at most one notification.
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if ( cVVector.size() > 0 ) {
                ...
                //delete values from database if its older than one day
                // delete old data so we don't build up an endless history
                // exmaple if date is 24 jan 2017 < 25 jan 2017 then delete by firing query
                // like these DELETE Uri from Table where yesterday_Date < = Current_Date;
                getContext().getContentResolver().delete(WeatherContract.WeatherEntry.CONTENT_URI,
                        WeatherContract.WeatherEntry.COLUMN_DATE + " <= ?",
                        new String[] {Long.toString(dayTime.setJulianDay(julianStartDay-1))});


                //notify the weather details
                notifyWeather();
            }
    ...
    private String appId(){
        return "9f09280a6dbeeb6eb2968ab165c8bfb5";
    }
